//CMPR131 - Chapter 6 
//Members: Thi Ho, Thi Ngoc Pham, Khang Quach, Minh & Thinh
//10/03/2025
...
void mainMenu()
{
	cout << "\n\tCMPR131 Chapter 6: non-template and template Container by Thi Ho, Thi Ngoc Pham, Khang Quach, Minh & Thinh (9/30/25)";
	cout << "\n\t" << string(100, char(205));
	...
	do
	{
		...
		case 'D':
		{
			if (dbag.isEmpty()) {
				...
				if (idx < 0 || idx >= dbag.getSize())
				{
					cout << "\n\t\tInvalid index. Please enter an index between 0 and " << dbag.getSize() - 1 << ".";
				}
				else
				{
					dbag.remove(idx);
...
void ApplicationFunction()
{
	...
	do {
		...
		case 2: { // read in data files
			...
			for (int i = 0; i < courseCount; ++i) {
				while (true) {
					...
					string stop = filename;

					for (int i = 0; i < stop.length(); i++)
					{
						stop[i] = toupper(stop[i]);
					}
		...
		case 3: { // search for a student record from a course
			...
			{
				cout << "\n\tERROR: No data file has been read and stored into Courses."; break;
			}
